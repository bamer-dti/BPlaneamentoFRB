package utils;

public class Campos {
    public static final String TABELA_OSBO = "tab_osbo";
    public static final String TABELA_OSBI = "tab_osbi";
    public static final String TABELA_OSPROD = "tab_osprod";
    public static final String TABELA_OSTIMER = "tab_ostimer";

    public static final String OBRANO = "obrano";
    public static final String FREF = "fref";
    public static final String NMFREF = "nmfref";
    public static final String REF = "ref";
    public static final String DESIGN = "design";
    public static final String QTT = "qtt";
    public static final String DIM = "dim";
    public static final String MK = "mk";
    public static final String FAMILIA = "familia";
    public static final String ESTADO = "estado";
    public static final String SECCAO = "seccao";
    public static final String OBS = "obs";
    public static final String DTCORTEF = "dtcortef";
    public static final String DTTRANSF = "dttransf";
    public static final String DTEMBALA = "dtembala";
    public static final String DTEXPEDI = "dtexpedi";
    public static final String DTCLIENTE = "dtcliente";
    public static final String BOSTAMP = "bostamp";
    public static final String BISTAMP = "bistamp";
    public static final String TIPO = "tipo";
    public static final String ORDEM = "ordem";
    public static final String COR = "cor";

    public static final String KEY_OSBI = "osbi03";
    public static final String KEY_OSBO = "osbo";
    public static final String KEY_OSPROD = "osprod";
    public static final String KEY_OSTIMER = "ostimer";
    public static final String KEY_SECCAO = "seccao";
    public static final String KEY_NOTAS = "notas";

    public static final String DOCTYPE_DEL_OSBI = "delosbi";
    public static final String ESTADO_01_CORTE = "01 - CORTE";
    public static final String ESTADO_00_APROV = "00 - APROV.";
    public static final String ESTADO_00_PLANEAMENTO = "00 - PLANEAMENTO";
    public static final String TEXTO = "texto";
    public static final String UNIXTIME = "unixtime";
    public static final String LASTTIME = "lasttime";
    public static final String MAQUINA = "maquina";
    public static final String POSICAO = "posicao";
    public static final String SECCAO_CARPINTARIA = "12 - CARPINTARIA";
    public static final String SECCAO_ALUMINIOS = "10 - ALUMINIOS";
    public static final String SECCAO_ACOS = "11 - AÇOS";
    public static final String NATUREZA_CORTE = "corte";
    public static final String _ID = "_id";
    public static final String NUMLINHA = "numlinha";
    public static final String OPERADOR = "operador";
}
